...
const Details: React.FC<ShopTransferDetailsProps> = ({ data, userId }) => {
  ...
  console.log(typeof data.balance_info?.payment_info);

  const paymentInfoArray = useMemo(() => {
    let newPaymentInfo: PaymentInfo[] = [];
    if (
      data.balance_info?.payment_info &&
      isString(data?.balance_info?.payment_info)
    ) {
      newPaymentInfo.push(JSON?.parse(data?.balance_info?.payment_info));
    }
      ...
      <div className="bg-[#F9FAFB] items-stretch rounded-md 2xl:p-12 p-4 3xl:px-[6.25rem] border border-dashed border-[#E5E7EB] grid grid-cols-5">
        <ShopDetailsCard
          ...
          data={{
            ...
            id: data?.shop?.id!,
            name: data?.shop?.name!,
            address: data?.shop?.address as any,
            description: data?.shop?.description!,
            socials: data?.shop?.settings?.socials as any,
          }}
        />
        <div className="2xl:col-span-1 col-span-full text-center self-center 2xl:mt-10 my-5">
          <span className="bg-accent w-9 h-9 rounded-full text-white inline-flex mb-3">
            <ArrowNext className="m-auto hidden 2xl:block" />
        ...
        <ShopDetailsCard
          ...
          data={{
            ...
            image: data?.current_owner?.profile?.avatar,
